<script lang="ts">
	...
	let { value = '', size = 'lg', children }: { value?: string; size?: 'sm' | 'lg'; children?: Snippet } = $props();
...
<div class="inline-flex items-stretch border-[2.5px] border-[#1a1a1a] rounded-md overflow-hidden bg-white shadow-sm select-none">
	<div
		class="flex flex-col items-center justify-center bg-[#2840b5] gap-1.5"
		style="min-width:{stripe.w}px;padding:{stripe.pad}"
	>
		<svg width={stripe.svgSize} height={stripe.svgSize} viewBox="0 0 {stripe.svgSize} {stripe.svgSize}" aria-hidden="true">
		...
		<span class="text-white font-bold leading-none" style="font-size:{stripe.textSize};letter-spacing:0.05em">H</span>
	</div>
	{#if children}
		{@render children()}
	{:else}
		<span
			...
			style="font-size:{size === 'sm' ? '1rem' : '1.8rem'};padding:{size === 'sm' ? '4px 10px' : '8px 14px'}"
		>
			{value}
		</span>
	{/if}
