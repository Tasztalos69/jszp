<script lang="ts">
	...
	const trafficOk = $derived(v.trafficStatus.toLowerCase().includes('helyez') || v.trafficStatus.toLowerCase().includes('forgalom'));
	const stolenOk  = $derived(v.stolenStatus.toLowerCase().includes('nem áll') || v.stolenStatus.toLowerCase().includes('nincs'));
...
<div class="min-h-dvh bg-slate-50">
	<!-- Topbar -->
	<header class="sticky top-0 z-10 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 py-3">
		<a href="/" class="flex items-center gap-1.5 text-sky-700 font-medium text-sm min-h-[44px] hover:underline">
		...
		>
			{#if copied}
				...
				Másolva
			{:else}
				<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
				Másolás
			{/if}
		</button>
	</header>
		...
		>
			...
			<div class="flex flex-col gap-1.5">
				<div class="flex flex-wrap items-center gap-x-4 gap-y-1">
					{#if v.year}
						<span class="bg-[#0f4c2a] text-white text-sm font-bold px-2.5 py-0.5 rounded">{v.year}</span>
					{/if}
					{#if v.displacement}
						<span class="text-sm font-semibold text-[#1a3a2a]">{v.displacement}</span>
					{/if}
					{#if v.powerKw}
						<span class="text-sm font-semibold text-[#1a3a2a]">{v.powerKw}{v.powerLe ? ` / ${v.powerLe} LE` : ''}</span>
					{/if}
				</div>
				<div class="flex flex-wrap items-center gap-x-4 gap-y-1">
					...
					{#if v.color || v.seats}
						<span class="text-sm text-[#2d5a3d]">{[v.color, v.seats ? `${v.seats} fő` : ''].filter(Boolean).join(', ')}</span>
					{/if}
				</div>
			</div>
		</div>
			...
			<dl class="divide-y divide-slate-100">
				{#if v.trafficStatus}
					<div class="flex items-center justify-between gap-3 px-4 py-3">
						<dt class="text-sm text-slate-500 shrink-0">Forgalmi állapot</dt>
